...
export const Route = createFileRoute("/TaskItem/TaskItemEdit/$id")({
...
function TaskItemEdit() {
  ...
  const [title, setTitle] = useState(tasks.data.title);
  const [description, setDescription] = useState(tasks.data.description);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/tasks/${tasks.data.id}`,
        {
          method: "PUT",
          headers: {
      ...
      const response = await res.json();
      if (res.status == 200) {
        await router.invalidate({ filter: (e) => e.routeId == "/" });
        router.navigate({ to: "/" });
      }
    } catch (e) {}
  };
  ...
  return (
    <div className="w-full  max-h-[90vh] flex justify-center p-10">
      <div className="rounded-4xl w-100 bg-(--primary-bg) p-5 text-gray-900">
        <h1 className="text-4xl text-center">Modificar Tarea</h1>
        <hr className="my-5" />
